...
/**
 * Готова таблиця найближчих днів (ISO + день тижня), щоб модель НЕ рахувала дати
 * сама (Haiku робить це ненадійно), а просто брала потрібний рядок.
 * Рахуємо в UTC із рядка дати, тож результат не залежить від таймзони сервера.
 */
function upcomingDays(todayIso: string, count: number): string {
  const [year, month, day] = todayIso.split('-').map(Number)
  const fmt = new Intl.DateTimeFormat('uk-UA', { weekday: 'long', timeZone: 'UTC' })
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const tag = i === 0 ? ' (сьогодні)' : i === 1 ? ' (завтра)' : ''
...
Календар найближчих днів (бери дати ЗВІДСИ, не рахуй сам):
${upcomingDays(today, 14)}

Правила:
- Одна окрема ДІЯ = одна задача. Кілька різних дій в одному реченні — кілька задач.
- АЛЕ одна дія з уточненням мети чи деталей — це ОДНА задача, а не дві: короткий заголовок дії + деталь у полі notes. Не дроби одну дію на кілька.
  Приклад: «зустрітись з мамою поговорити про важливі справи» → title=«Зустрітися з мамою», notes=«Поговорити про важливі справи». НЕ дві окремі задачі.
...
- notes — це підзаголовок для уточнення однієї задачі (мета, контекст, «щоб…», «поговорити про…»). Додавай лише коли така деталь є в тексті.
...
- dueDate став ТІЛЬКИ якщо в тексті явно згадано час. Формат строго YYYY-MM-DD. Якщо часу не згадано — НЕ додавай dueDate.
  • Назва дня тижня («в неділю», «у вівторок», «в пятницю») → знайди в календарі вище НАЙБЛИЖЧИЙ майбутній рядок із цим днем тижня і візьми його дату.
  • Відносне формулювання («завтра», «післязавтра», «до п'ятниці») → бери відповідний рядок із календаря.
  • Конкретна дата («31 липня», «5 серпня», «31.07», «15-го») → постав саме цю дату у форматі YYYY-MM-DD (рік ${today.slice(0, 4)}, якщо не сказано інший; якщо місяць уже минув — наступний рік). День тижня рахувати не треба.
- Сьогоднішню дату (${today}) став ЛИШЕ тоді, коли користувач явно сказав «сьогодні» або очевидний синонім («сьогодні ввечері», «сьогодні вранці», «до кінця дня»). Без слова «сьогодні» НЕ став сьогоднішню дату за замовчуванням.
  Приклад: «зустрітись з мамою» → без dueDate. «сьогодні зустрітись з мамою» → dueDate ${today}.
- Не додавай задач, яких немає в тексті. Не об'єднуй різні дії в одну і не дроби одну дію на кілька.
